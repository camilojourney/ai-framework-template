...
Load `contexts/{{PROJECT_SLUG}}.md` for full product context.
...
**Symptoms:** `{{TEST_CMD}}` exits with errors
...
2. Different {{PACKAGE_MANAGER}} versions
...
3. Clear local cache and rebuild: `{{PACKAGE_MANAGER}} clean-install`
...
5. Lock file needs regeneration: `{{PACKAGE_MANAGER}} install`
...
|--------|--------|-------------|
| Test coverage | ≥80% on new code | `{{TEST_CMD}}:coverage` |
| Lint errors | 0 new errors | `{{LINT_CMD}}` |
| Type errors | 0 | `{{BUILD_CMD}}` |
| Build time | {{BUILD_TIME_TARGET}} | CI logs |
